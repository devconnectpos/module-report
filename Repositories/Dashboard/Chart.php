...
class Chart extends Graph
{
    ...
    public function __construct(
        ...
        ReportOrderCollectionFactory $orderReportCollectionFactory,
        array $data = []
    ) {
        ...
        foreach ($dates as $d) {
            foreach ($this->getAllSeries() as $index => $serie) {
                if ($index == 'list_customer') {
                    ...
                    // Vì các axis và allSeries đều lấy từ một nguồn nên sẽ có thứ tự giống nhau, func aray_shift cắt phần từ đầu tiên.
            ...
            foreach ($items as $item) {
                if ($single) {
                    if (strtotime($dateStart) <= strtotime($item->getData('range'))
                        && strtotime($item->getData('range')) <= strtotime($dateEnd)) {
                        if ($attributes == "range") {
